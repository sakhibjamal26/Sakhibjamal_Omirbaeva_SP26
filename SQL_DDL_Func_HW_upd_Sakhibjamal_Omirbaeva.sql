...
/*
  ...
  Note on current dvdrental data:
    This database contains payments only from 2017 (Q1 and Q2).
    So running SELECT * FROM sales_revenue_by_category_qtr today (2026)
    returns 0 rows — this is expected, not a bug in my view.
    I verified the logic by running a manual query against Q2 2017 (test 2 below).
    I got 16 rows. The top category was Sports with $14,306.22, followed by
    Animation ($13,864.00) and Sci-Fi ($13,018.46). The lowest was Music at $9,680.57.
    All 16 categories had at least one sale so all passed the HAVING filter.

  Schema note:
    I create this view explicitly in the public schema. Tasks 3 and 4 use core.
    I am being explicit here so there is no ambiguity about where each object lives.

  Example of data that should NOT appear:
    Any category with zero payments in the current quarter won't appear.
    For instance, if I query Q1 2017, categories with no activity in that quarter
    are correctly excluded — the INNER JOIN chain and HAVING both enforce this.
*/
...
-- I ran this and got 0 rows — correct, the database has no 2026 payments.

-- Test 2 — I verify the logic manually using Q2 2017 (the actual data in this database):
...
HAVING SUM(p.amount) > 0
ORDER BY total_revenue DESC;
-- I ran this and got 16 rows. Top: Sports $14,306.22 | Animation $13,864.00 | Sci-Fi $13,018.46.
-- All 16 categories had sales in Q2 2017 so all passed the HAVING > 0 filter.
-- Zero-sales exclusion is confirmed — any category with no payments in a quarter won't appear.
...
-- Test 1 — valid input with data (Q2 2017):
SELECT * FROM public.get_sales_revenue_by_category_qtr('2017-04-01');
-- I ran this and got 16 rows, same result as the view verification — Sports $14,306.22 on top.

-- Test 2 — edge case: current date (no 2026 data):
SELECT * FROM public.get_sales_revenue_by_category_qtr(CURRENT_DATE);
-- I ran this and got 0 rows — correct, no exception raised.

-- Test 3 — NULL input:
SELECT * FROM public.get_sales_revenue_by_category_qtr(NULL);
-- I ran this and got 0 rows — NULL in WHERE conditions matches nothing, safe result.
...
/*
  ...
  Case sensitivity:
    I wrap both sides in UPPER() so 'united states', 'United States', and 'UNITED STATES'
    all return the same result. Without this, the function silently returns nothing
    for non-exact casing which would be confusing for the caller.
*/
...
BEGIN
    ...
    WITH film_rentals_by_country AS (
        ...
        -- I use unambiguous aliases to avoid PL/pgSQL conflicts with return column names
        SELECT
            ...
            f.release_year::INTEGER             AS rel_year,
        ...
        WHERE UPPER(co.country) = ANY(SELECT UPPER(c) FROM UNNEST(countries) AS c)
    ...
    ranked AS (
        ...
        SELECT *,
            ROW_NUMBER() OVER (
                ...
                ORDER BY rental_count DESC, revenue DESC, film_title
            ) AS rn
...
/*
  ...
  What "in stock" means:
    A film is in stock only if at least one of its inventory copies is currently
    available — meaning there is no active rental (return_date IS NULL) for that copy.
    I filter this out with: AND i.inventory_id NOT IN (SELECT inventory_id FROM rental
    WHERE return_date IS NULL). Without this, the function would return all films
    matching the title pattern regardless of whether any copy is actually on the shelf.

  How rental_date is determined:
    I show the most recent rental date for each film across all its copies,
    including unreturned (active) rentals — those are likely the most recent ones.
    I use ORDER BY rental_date DESC to pick the latest.
  ...
  How row_num is generated:
    I use a simple integer counter variable that I increment inside a FOR loop.
    I don't use ROW_NUMBER() OVER (window function) as the task requires a simpler approach.

  What happens if multiple matches:
    All matching in-stock films are returned, each with a unique row_num starting from 1.
...
BEGIN
    ...
    -- I check how many distinct films match the pattern AND have at least one copy in stock
    SELECT COUNT(DISTINCT f.film_id) INTO v_count
    FROM film f
    JOIN inventory i ON f.film_id = i.film_id
    WHERE f.title ILIKE title_pattern
      AND i.inventory_id NOT IN (
          -- I exclude inventory copies that are currently checked out (no return yet)
          SELECT inventory_id FROM rental WHERE return_date IS NULL
      );

    -- If no in-stock matches found, I return a single informational row instead of an error
    IF v_count = 0 THEN
        row_num       := NULL;
        film_title    := 'No films found matching: ' || title_pattern;
        language      := NULL;
        customer_name := NULL;
        rental_date   := NULL;
        RETURN NEXT;
    ...
    -- I use a FOR loop and a manual counter instead of ROW_NUMBER() OVER
    FOR v_rec IN (
        WITH available_films AS (
            -- Films that have at least one copy currently available (not rented out)
            SELECT DISTINCT
                f.film_id,
                f.title  AS f_title,
                l.name   AS lang
        ...
        last_rental_per_film AS (
            -- I find the most recent rental per film across all copies.
            -- I include active (unreturned) rentals because they're likely the most recent.
        ...
        SELECT
            af.f_title,
            af.lang,
            COALESCE(lr.cust_name, 'Never rented') AS cust_name,
            lr.r_date
        FROM available_films af
        LEFT JOIN last_rental_per_film lr ON af.film_id = lr.film_id
        ORDER BY af.f_title
    ) LOOP
        v_counter     := v_counter + 1;
        row_num       := v_counter;
        film_title    := v_rec.f_title;
        language      := v_rec.lang;
        customer_name := v_rec.cust_name;
        rental_date   := v_rec.r_date;
        RETURN NEXT;
    END LOOP;
...
/*
  ...
  How I generate unique film_id:
    I use SELECT nextval('film_film_id_seq') which is the sequence already attached
    to the film table. This is safe under concurrent access — two sessions calling
    this at the same time each get a different value from the sequence.
    Using MAX(film_id) + 1 instead would be unsafe because two sessions could both
    read the same MAX and try to insert the same ID.

  How I ensure no duplicates:
    Before anything else, I check for an existing film with the same title.
    I use UPPER() on both sides so 'THE MATRIX' and 'The Matrix' are treated as
    duplicates — titles that differ only in capitalisation should not both be inserted.
  ...
  How I validate language existence:
    I query the language table using UPPER() on both sides so 'klingon', 'KLINGON',
    and 'Klingon' all match. If nothing is found the language doesn't exist and
    I raise an exception. This prevents inserting a film with an invalid language reference.
...
BEGIN
    -- I check for duplicate title using UPPER() on both sides — 'THE MATRIX' and 'The Matrix'
    -- should be treated as duplicates, not two separate films
    ...
    -- I look up the language_id using UPPER() so 'klingon' / 'KLINGON' / 'Klingon' all match
    ...
    -- I use the existing sequence for film_id — this is safe under concurrent access.
    -- Using MAX(film_id) + 1 would be unsafe because two sessions could both read
    -- the same MAX and try to insert the same ID at the same time.
