...
-- Solution 1: JOIN
SELECT CASE
           WHEN NULLIF(a.address2, '') IS NOT NULL THEN a.address || ', ' || a.address2
           ELSE a.address
       END AS full_address,
...
-- Mentor noted that using CONCAT+COALESCE produces a trailing comma like "123 Main St, "
-- when address2 is NULL, so I switched to CASE WHEN NULLIF to handle that properly.
...
-- Mentor noted that CONCAT+COALESCE gives a trailing comma when address2 is NULL,
-- so I fixed it with CASE WHEN NULLIF across all three solutions.
...
-- This works but the same subquery is written twice — once in SELECT, once in WHERE.
-- Mentor noted this is a bad pattern because the database runs the same calculation
-- twice per row, which is wasteful and slow on big tables.
-- In production this should definitely be replaced with the CTE or JOIN version.
...
-- Solution 2: Subquery
-- Mentor noted that my original correlated subquery approach (3 subqueries per row) was
-- very slow and not acceptable. The correct approach is CASE WHEN inside COUNT even here.
-- I rewrote this to use a subquery that pre-joins film with categories,
-- then applies CASE WHEN in the outer query — same logic as Solution 1 but structured
-- with a subquery to show a different way of organizing the code.
SELECT release_year,
       COUNT(CASE WHEN genre = 'Drama' THEN film_id END) AS number_of_drama_movies,
       COUNT(CASE WHEN genre = 'Travel' THEN film_id END) AS number_of_travel_movies,
       COUNT(CASE WHEN genre = 'Documentary' THEN film_id END) AS number_of_documentary_movies
FROM (
    SELECT f.film_id,
           f.release_year,
           c.name AS genre
    FROM public.film f
    LEFT JOIN public.film_category fc ON f.film_id = fc.film_id
    -- LEFT JOIN so films with no category are still included in the year count
    LEFT JOIN public.category c ON fc.category_id = c.category_id
) AS film_genres
GROUP BY release_year
ORDER BY release_year DESC;

-- The subquery flattens film + category into one result set,
-- then the outer query groups by year and uses CASE WHEN to pivot genres into columns.
-- Much faster than correlated subqueries — only one pass through the joined data.
-- Mentor noted this is the correct and efficient approach for this kind of pivot.
...
INNER JOIN (
    ...
    -- using DISTINCT to avoid duplicate rows if two payments share the exact same MAX timestamp
...
-- Mentor noted that if two payments share the exact same MAX timestamp, the subquery
-- returns multiple rows and the join duplicates that staff member in the result.
-- I added DISTINCT inside the subquery to fix that edge case.
...
-- I originally hardcoded 2017 as the current year since it's the last year in the dataset.
-- Mentor noted that hardcoding years is not robust — if new data is added the query breaks.
-- Fixed by using (SELECT MAX(release_year) FROM public.film) dynamically instead.

-- Solution 1: JOIN
SELECT a.first_name,
       a.last_name,
       MAX(f.release_year) AS last_film_year,
       ((SELECT MAX(release_year) FROM public.film) - MAX(f.release_year)) AS years_inactive
FROM public.actor a
INNER JOIN public.film_actor fa ON a.actor_id = fa.actor_id
INNER JOIN public.film f ON fa.film_id = f.film_id
GROUP BY a.actor_id, a.first_name, a.last_name
...
-- Simple, works well. Dynamic max year means this stays correct if the dataset changes.

-- Solution 2: Subquery
SELECT a.first_name,
       a.last_name,
       last_film.last_year,
       ((SELECT MAX(release_year) FROM public.film) - last_film.last_year) AS years_inactive
FROM public.actor a
INNER JOIN (
    SELECT fa.actor_id,
...
current_year AS (
    -- get the latest release year in the dataset dynamically instead of hardcoding
...
SELECT a.first_name,
       a.last_name,
       lf.last_film_year,
       (cy.max_year - lf.last_film_year) AS years_inactive
FROM public.actor a
INNER JOIN last_film_per_actor lf ON a.actor_id = lf.actor_id
CROSS JOIN current_year cy
ORDER BY years_inactive DESC;

-- CTE version is the clearest. I added a second CTE to get the max year dynamically.
-- CROSS JOIN with current_year is fine here because it's a single-row result.
