...
-- verify what we are about to insert before committing
...
INSERT INTO public.film (film_id, title, description, release_year, language_id,
                         ...
                         rating, last_update)
SELECT
    (SELECT MAX(film_id) + 1 FROM public.film),
    'Inception',
...
-- verify which customer we are about to update before committing
...
WHERE c.customer_id = (
    ...
    WHERE (SELECT COUNT(*) FROM public.rental r WHERE r.customer_id = c2.customer_id) >= 43
      AND (SELECT COUNT(*) FROM public.payment p WHERE p.customer_id = c2.customer_id) >= 43
